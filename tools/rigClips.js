// The rig animation model — the schema the Rig Studio edits, its evaluator, and the
// built-in clips that reproduce what the game does TODAY.
//
// This file has ONE copy. tools/build_rig_studio.py inlines it into the studio (which
// is a single <script> in a file:// page, not a module), and src/rigClips.test.ts drives
// it against the real src/raid/EnemyActor.ts and src/raid/RaidActor.ts, pose for pose.
// That is the point of pulling it out of the template: a bench that animates a rig
// differently from the game teaches you a wrong animation, and the mistake ships looking
// measured. The same arrangement tools/tileAnchorGeometry.js has with Field.
//
// ---------------------------------------------------------------------------
...
const SWING_FRAC = 0.72, REST_FRAC = 1 - SWING_FRAC;
...
export { SWING_FRAC, REST_FRAC, AUTHORED };
