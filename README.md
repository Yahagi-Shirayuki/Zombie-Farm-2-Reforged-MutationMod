...
- Headless zombies can't wear head or hair/eye mutations — with one exception that runs the other way: **Pumpking** (the level-23 crop) is headless-only, giving them the head they never had (+3 attack). Every route enforces the split, client and server: crop adjacency, the Zombie Pot (each parent is filtered to the child's body type first, so a garlic head isn't lost to a Pumpking parent it would only shed), and every mask that lands on a unit.
...
- **Friends**: friend codes, server-backed friend lists, **daily gifting** (sending awards 5 XP; the first two sends per day are free, sends 3–10 cost 100 gold each, and each friend can receive at most one per day), and a **gift inbox** with claiming. A gift's contents are rolled by the server the moment it is *sent* and stored on it — nothing the recipient does can re-roll them — and stay hidden until it is opened: 10% a brain, otherwise 150 / 300 / 500 / 1000 gold (25/25/25/15%). The **first gift each player opens each UTC day is always a brain**, whatever it was rolled as.
