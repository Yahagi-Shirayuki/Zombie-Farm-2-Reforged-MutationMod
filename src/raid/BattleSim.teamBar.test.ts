...
//
// It did not. The scene divided the live HP sum by a constant captured at construction
// from the roster CombatEngine handed over, and that roster's con already carries the
// full team aura, while BattleSim pays the aura only to zombies that have DEPLOYED. An
...
describe("the enemy bar measures the WAVE, not the boss's furniture", () => {
  ...
  /** A boss that does nothing but conjure minions, as fast as the cap allows. */
  const summonOnly = (summonTemplate: CombatUnit | null, wallTemplate: CombatUnit | null, name: string) =>
    new BattleSim(
      harmless(),
      [
      ...
      null, true,
      [{ name, weight: 1, castMs: 0, cooldownMs: 100, damage: 0 }],
      undefined, summonTemplate, wallTemplate,
    );

  const runUntilSpawned = (sim: BattleSim): SimUnit[] => {
    for (let i = 0; i < 80 && !sim.units.some((u: SimUnit) => u.isSpawned); i++) sim.step(50);
    return sim.units.filter((u: SimUnit) => u.isSpawned);
  };

  it("a summoned minion moves neither side of the enemy bar", () => {
    const sim = summonOnly(enemy({ id: "minion", con: 50 }), null, "summonBoss");
    const before = sim.teamTotals();
    const summoned = runUntilSpawned(sim);
  ...
  it("a summoned blocker does the same", () => {
    // A sourceKey of its own: the boss refuses to raise a second blocker while one of
    // the SAME species is standing, and the wave's own enemy would otherwise count.
    const sim = summonOnly(
      null, enemy({ id: "wall", sourceKey: "FarmStageActorCarrotWall", con: 80 }), "wall",
    );
    const before = sim.teamTotals();
    const walls = runUntilSpawned(sim);
    expect(walls.length).toBeGreaterThan(0);
    expect(walls.every((u) => u.isWall)).toBe(true);

    const after = sim.teamTotals();
    expect(after.enemyMax).toBe(before.enemyMax);
    expect(after.enemyHp).toBe(before.enemyHp);
    expect(after.enemyAlive).toBe(before.enemyAlive + walls.length);
  });
});
