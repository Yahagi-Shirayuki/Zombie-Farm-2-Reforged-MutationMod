...
export function buildPlayerUnits(
  ...
  return rows.map(({ z, keys, eff }) => {
    ...
    const powder = sanitizeZombiePowderStats(z.powderStats);
    ...
    const focusPowder = wisToFocusBonus(powder.white ?? 0);
 ...
 *
 *  Extracted verbatim from buildEnemyUnits — same body, same order, no behaviour
 *  change — so the stage modules can build a roster without a RaidStage in hand. */
