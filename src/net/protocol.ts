...
export type GameplayCommand =
  ...
  | {
      ...
      cropKey: string;
      // `variant` is this fork's: a crop with more than one authored look picks one at
      // plant time, and the server records it so both sides draw the same plot.
      plots: { oc: number; or: number; fertilized?: boolean; variant?: number }[];
    }
