...
export class ZombieField {
  ...
  spawn(key: string, col: number, row: number, mutation?: MutationInput, mutationIds?: readonly string[]): ZombieUnit | null {
    ...
    const mutationSet = splitMutationInput(mutation, mutationIds);
    const data = makeOwned(`z${this.nextId++}`, def, col, row, 0, mutationSet.mask, undefined, undefined, mutationSet.ids);
  ...
  spawnVerified(
    key: string, col: number, row: number, mutation?: MutationInput, mutationIds?: readonly string[]
  ): ZombieUnit | OwnedZombie | null {
    this.harvesting = true;
    try {
      if (this.canAdd()) return this.spawn(key, col, row, mutation, mutationIds);
  ...
  combine(idA: string, idB: string, baseDurationMs?: number, potId?: string): boolean {
    ...
    return pot.start(
      ...
      { id: a.id, key: a.key, name: a.name, mutation: a.mutation, mutationIds: a.mutationIds, color: this.colorOf(a), ...this.speciesTraits(a) },
  ...
  combinePreview(potId?: string): {
    key: string; name: string; mutation: number; mutationIds?: string[]; color?: [number, number, number];
  } | null {
  ...
  collectCombine(
    ...
    const data = makeOwned(
      `z${this.nextId++}`, def, col, row, 0, mutations.mask, result.color, result.name, mutations.ids
    );
