...
// SCOPE: the VANILLA catalog only — MUTATION_LIST, the bit-addressed mutations.
//
// The modded string-id mutations share these same slots and are NOT checked here,
// because they do not share the ladder: refRank in mutations.ts scores every modded id
// above every vanilla bit, so a modded mutation wins its slot in the Pot regardless of
// what it pays. Under that rule the level-39 Pumpking loses its slot to a Turnip-head.
// Whether that is a bug or the point of the mod is a design call, not one this file can
// make — so it checks the ladder that IS a ladder, and says plainly that it stops there.
import { describe, expect, it } from "vitest";
import { MUTATION_LIST, SLOTS, type MutationDef, type Stat } from "./mutations";

// EVERY stat a mutation can move, wis included. Leaving wis out does not just miss
// findings, it INVENTS them: Broccohair and Cauli-hair differ only in con and wis, so a
// str/dex/con comparison reports Broccohair as dominating a mutation it actually trades
// with.
const STATS: Stat[] = ["str", "dex", "con", "wis"];
