...
export class Hud {
  ...
  getPotStatus:
    | (() => {
        ...
        canStore: boolean;
        pending: {
          keyA: string; keyB: string; maskA: number; mutationIdsA?: string[]; maskB: number; mutationIdsB?: string[];
          colorA?: [number, number, number]; colorB?: [number, number, number];
        } | null;
        /** The finished zombie, set only once the combine is ready to collect. */
        result: {
          key: string; name: string; mutation: number; mutationIds?: string[];
    ...
    for (const z of eligible) {
      ...
      if (this.zombieMutationPortraitOf) {
        onFirstVisible(portrait, () => {
          void this.zombieMutationPortraitOf?.(z.key, z.mutation, z.color, z.mutationIds)
  ...
  openCombiner() {
    ...
    const showPortrait = (
      ...
      onFirstVisible(el, () => {
        void this.zombieMutationPortraitOf?.(key, mutation, color, mutationIds)
          .then((portrait) => {
    ...
    const renderBusy = () => {
      ...
      if (done) {
        ...
        showPortrait(p, st.result!.key, st.result!.mutation, st.result!.color, st.result!.mutationIds);
        const n = document.createElement("div");
        n.className = "cmb-rn";
        n.textContent = st.result!.name;
        const mut = document.createElement("div");
        mut.className = "cmb-rm";
        mut.textContent = mutationLabel(st.result!.mutation, st.result!.mutationIds) || "no mutations";
        ...
        const parent = (key: string, mask: number, color?: [number, number, number], mutationIds?: readonly string[]) => {
          const d = document.createElement("div");
          d.className = "cmb-slot filled";
          const p = document.createElement("div");
          p.className = "cmb-por";
          showPortrait(p, key, mask, color, mutationIds);
          const mut = document.createElement("div");
          mut.className = "cmb-sm";
          mut.textContent = mutationLabel(mask, mutationIds) || "no mutations";
    ...
    const renderIdle = () => {
      ...
      const slotEl = (which: "A" | "B") => {
        ...
        if (z) {
          const p = document.createElement("div");
          p.className = "cmb-por";
          showPortrait(p, z.key, z.mutation, z.color, z.mutationIds);
          const n = document.createElement("div");
          n.className = "cmb-sn";
          n.textContent = z.name;
          const mut = document.createElement("div");
          mut.className = "cmb-sm";
          mut.textContent = mutationLabel(z.mutation, z.mutationIds) || "no mutations";
      ...
      if (roster.length < 2) {
        ...
        for (const z of roster) {
          ...
          if (z.mutation || z.mutationIds?.length) {
            const m = document.createElement("div");
            m.className = "cmb-zmut";
            m.textContent = "M";
            m.title = mutationLabel(z.mutation, z.mutationIds);
            c.appendChild(m);
          }
          if (!chosen && eligible) {
            c.onclick = () => {
