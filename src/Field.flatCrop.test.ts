...
const makeField = (): CropLayout => {
  ...
  Object.assign(field, {
    ...
    // This fork splits a grown crop across cropGroundLayer (its baked dirt) and
    // cropEntityLayer (the plants-only copy) so a plot's dirt cannot clip the crop
    // behind it. Upstream parents the whole sprite to entityLayer; both are stubbed
    // so the test reads whichever the build actually uses.
...
const layerOfGrown = (cfg: CropConfig): "seed" | "entity" | "other" => {
  ...
  if (sprite.parent === field.cropEntityLayer || sprite.parent === field.entityLayer) return "entity";
  return "other";
};
...
describe("flat crops", () => {
  ...
  it("draws a Water Lily flat while it GROWS, and only while it grows", () => {
    ...
    for (const stage of cfg.stages) {
      ...
      // Ripe graduates into the depth sort — cropEntityLayer in this build (see the
      // stub above), entityLayer upstream. Growing stays flat in cropSeedLayer.
      const depthSorted = sprite.parent === field.cropEntityLayer || sprite.parent === field.entityLayer;
      expect(stage === ripe ? depthSorted : sprite.parent === field.cropSeedLayer,
        `stage ${stage} landed in the wrong layer`).toBe(true);
    }
  });
